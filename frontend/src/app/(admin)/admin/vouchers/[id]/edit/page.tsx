...
import { ArrowLeft, Save, Loader2, Tag, Percent, Banknote, ShoppingCart, ShoppingBag, Gift, Calendar, CheckSquare, RefreshCw } from 'lucide-react'
    ...
    const fetchData = async () => {
      try {
        ...
        setForm({
          ...
          start_date: v.start_date.substring(0, 16), // datetime-local format
          end_date: v.end_date.substring(0, 16),
          tier_restriction: v.tier_restriction,
          is_active: v.is_active,
        })
      } catch (err: any) {
        toast.error('Không tìm thấy mã giảm giá hoặc lỗi đường truyền')
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-32 text-slate-400">
        <RefreshCw className="w-10 h-10 text-[#ed2a2a] animate-spin mb-4" />
        <span className="font-bold text-slate-500 uppercase tracking-widest text-xs">Đang tải dữ liệu...</span>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm font-medium text-slate-500 mt-1">Cập nhật cấu hình khuyến mãi hiện tại.</p>
          </div>
        </div>
        <button
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Giới hạn & Thời gian */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full pb-20 sm:pb-0">
          ...
          <div className={cardCls}>
            <div className="space-y-5">
              ...
              <div className="pt-4 border-t border-slate-100">
                <label className={labelCls}>Phân khúc Khách hàng</label>
                <div className="grid grid-cols-2 gap-2">
                  {([
                    { value: 'all',    label: 'Tất cả TV',    cls: 'border-slate-400 bg-slate-100 text-slate-700' },
                    ...
                    <button
                      ...
                      className={`py-2 px-1 rounded-xl border-2 text-[12px] font-bold transition-all active:scale-95 ${
          ...
          <div className={cardCls}>
             <div className="space-y-6">
                ...
                <div className="space-y-5">
                  <div>
                    <label className={labelCls}>Ngày bắt đầu <span className="text-[#ed2a2a]">*</span></label>
                    <input
                      type="datetime-local"
                  ...
                  <div>
                    <label className={labelCls}>Ngày kết thúc <span className="text-[#ed2a2a]">*</span></label>
                    <input
                      type="datetime-local"
                      value={form.end_date}
                ...
                <div className="pt-4 border-t border-slate-100">
                   <label className={labelCls}>Trạng thái kích hoạt</label>
                   <div className="flex gap-2">
                      <button
                        type="button"
                        onClick={() => set('is_active', true)}
                        className={`flex-1 py-3 rounded-xl border-2 text-sm font-bold transition-all active:scale-95 ${
                          form.is_active ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-slate-100 text-slate-400 bg-slate-50'
                        }`}
                      >
                        Active
                      </button>
                      <button
                        ...
                        className={`flex-1 py-3 rounded-xl border-2 text-sm font-bold transition-all active:scale-95 ${
                          !form.is_active ? 'border-amber-500 bg-amber-50 text-amber-700' : 'border-slate-100 text-slate-400 bg-slate-50'
                        }`}
                      >
                        Disabled
                      </button>
