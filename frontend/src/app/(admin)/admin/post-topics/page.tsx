...
export default function PostTopicsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const res = await postTopicService.getAll({
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await postTopicService.delete(id)
  ...
  const handleToggle = async (id: number) => {
    try {
      await postTopicService.toggle(id)
      toast.success('Đã update trạng thái')
      load()
    } catch {
  ...
  const inactiveCount = topics.filter(t => t.status === 'inactive').length
    ...
    <div className="space-y-6 pb-24 lg:pb-10 relative">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-800 flex items-center gap-2">
            <BookOpen className="w-6 h-6 text-[#ed2a2a]" />
            Chủ Đề Bài Viết
          </h1>
      ...
      <div className="flex sm:grid sm:grid-cols-3 gap-3 overflow-x-auto scrollbar-hide pb-2 sm:pb-0">
        ...
        <div className="bg-green-50 rounded-[1.5rem] border border-green-200/60 p-5 shadow-sm min-w-[140px] flex-shrink-0">
          ...
          <p className="text-xs font-bold text-emerald-600/70 uppercase tracking-widest mt-1">Đang Hiển Thị</p>
        </div>
        <div className="bg-slate-50 rounded-[1.5rem] border border-slate-200/60 p-5 shadow-sm min-w-[140px] flex-shrink-0">
          <p className="text-3xl font-black text-slate-400 tracking-tight">{inactiveCount}</p>
          <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Đang Tạm Ẩn</p>
        </div>
      </div>
      ...
      <div className="sticky top-[72px] lg:top-4 z-20 bg-white/80 backdrop-blur-xl rounded-2xl shadow-sm border border-slate-200/60 p-4 sm:p-5 flex flex-col xl:flex-row flex-wrap gap-3">
        ...
        <div className="flex gap-2 overflow-x-auto whitespace-nowrap scrollbar-hide pb-1 sm:pb-0 w-full xl:w-auto shrink-0">
           {(['all', 'active', 'inactive'] as const).map(tab => (
              ...
              >
                 {tab === 'all' ? 'Tất cả' : tab === 'active' ? '🟢 Đang hiển thị' : '⚫ Tạm ẩn'}
              </button>
           ))}
        ...
        {loading && topics.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 bg-white rounded-[1.5rem] border border-slate-200 shadow-sm text-slate-400 text-center px-4">
             <div className="w-24 h-24 bg-slate-50 flex items-center justify-center rounded-full mb-4 ring-8 ring-slate-50/50">
                <BookOpen className="w-10 h-10 text-[#ed2a2a] opacity-50" />
             </div>
             <p className="font-black text-slate-700 text-[18px] mb-1">Chưa có chủ đề nào!</p>
             <p className="text-[14px] font-medium text-slate-400 mb-6 max-w-md">Bắt đầu xây dựng nội dung blog bằng cách thêm một chủ đề bài viết mới.</p>
          </div>
        ) : (
          <>
            {/* ── DESKTOP: TABLE ── */}
            <div className="hidden lg:block bg-white/50 backdrop-blur-md rounded-[1.5rem] shadow-sm border border-slate-200 overflow-hidden min-h-[400px]">
              <table className="hidden lg:table w-full text-sm">
                <thead className="bg-slate-50/95 backdrop-blur-md border-b border-slate-200 sticky top-0 z-10">
                  <tr>
                    <th className="px-5 py-4 w-10 text-center text-slate-500 font-bold uppercase text-[11px]">ID</th>
                    <th className="px-5 py-4 text-left font-black text-slate-500 uppercase tracking-widest text-[11px]">Chủ đề / Đường dẫn</th>
                    <th className="px-5 py-4 text-center font-black text-slate-500 uppercase tracking-widest text-[11px]">Bài Viết</th>
                    <th className="px-5 py-4 text-left font-black text-slate-500 uppercase tracking-widest text-[11px]">Ngày Tạo</th>
                    <th className="px-5 py-4 text-center font-black text-slate-500 uppercase tracking-widest text-[11px] w-36">Hiện Trạng</th>
                ...
                <tbody className="divide-y divide-slate-100 bg-white">
                  {topics.map(topic => (
                    <tr key={topic.id} className="hover:bg-slate-50/80 transition-colors">
                      <td className="px-5 py-4 text-center font-medium text-slate-400">#{topic.id}</td>
                      <td className="px-5 py-4">
                        <p className="font-bold text-slate-800 text-[15px]">{topic.name}</p>
                        <div className="text-[12px] font-semibold text-slate-400 mt-1 flex items-center gap-1.5 line-clamp-1 break-all">
                           /{topic.slug}
                        </div>
                      </td>
                      <td className="px-5 py-4 text-center">
                          <span className="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg text-[13px] font-black bg-blue-50 text-blue-600 border border-blue-100">
                             <FileText className="w-4 h-4" /> {topic.posts_count}
                          </span>
                      </td>
                      <td className="px-5 py-4">
                        <span className="font-semibold text-slate-600 text-[13px]">
                           {fmtDate(topic.created_at)}
                        </span>
                      </td>
                      <td className="px-5 py-4 text-center">
                        {topic.status === 'active' ? (
                          <span className="inline-flex items-center justify-center gap-1.5 text-[11px] px-3 py-1.5 rounded-full font-bold bg-green-50 text-green-700 border border-green-200">
                            Hiển Thị
                          </span>
                        ) : (
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center justify-center gap-2">
                          <button
                            ...
                            className={`p-2 rounded-xl border transition-all active:scale-95 ${topic.status === 'active' ? 'bg-emerald-50 border-emerald-200 text-emerald-600 hover:bg-emerald-100' : 'bg-slate-50 border-slate-200 text-slate-400 hover:bg-slate-100 hover:text-slate-600'}`}
                            title={topic.status === 'active' ? 'Ẩn' : 'Hiện'}
                          >
                            {topic.status === 'active' ? <Power className="w-[18px] h-[18px]" /> : <PowerOff className="w-[18px] h-[18px]" />}
                          </button>
                          <button
            ...
            <div className="block lg:hidden space-y-4">
               {topics.map(topic => (
                 <div key={topic.id} className="relative bg-white rounded-2xl border border-slate-200 p-5 shadow-[0_2px_8px_rgba(0,0,0,0.04)] transition-all duration-300 active:scale-[0.98]">
                             ...
                             <div className="flex items-center justify-between w-full">
                               {topic.status === 'active' ? (
                                  ...
                                  <span className="flex items-center gap-1.5 text-slate-500 text-[11px] font-black uppercase tracking-wider">
                                     <div className="w-2 h-2 rounded-full bg-slate-300"></div> Đang ẩn
                                  </span>
                               )}
                             </div>
                             
                             <div className="flex flex-col mt-1">
                               <h3 className="font-black text-[18px] text-slate-900 leading-tight">{topic.name}</h3>
                               <span className="text-sm font-semibold text-slate-400 mt-1 block">/{topic.slug}</span>
                             </div>
                             
                             <div className="text-[12px] font-bold text-blue-600 bg-blue-50 px-2.5 py-1 rounded-md w-fit flex items-center gap-1.5 mt-2 border border-blue-100/50">
                               <FileText className="w-3.5 h-3.5" /> {topic.posts_count} Bài viết
                             </div>
                          </div>

                          {/* 3 Dots Menu Xổ Giảm cho Mobile */}
                          <div className="relative action-menu-container shrink-0">
                             <button onClick={() => setOpenMenuId(openMenuId === topic.id ? null : topic.id)} className="p-2 -mr-3 -mt-2 text-slate-400 hover:text-slate-800 transition-colors">
                             ...
                             {openMenuId === topic.id && (
                                <div className="absolute right-0 top-10 w-48 bg-white rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.15)] border border-slate-100 z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                                   <button onClick={() => { handleToggle(topic.id); setOpenMenuId(null) }} className="w-full flex items-center gap-3 px-4 py-4 text-[15px] font-bold text-slate-700 hover:bg-slate-50 border-b border-slate-50 transition-colors first:rounded-t-2xl">
                                     {topic.status === 'active' ? <PowerOff className="w-5 h-5 text-slate-400" /> : <Power className="w-5 h-5 text-emerald-500" />} 
                                     {topic.status === 'active' ? 'Ẩn chủ đề' : 'Hiện chủ đề'}
                                   </button>
                                   <button onClick={() => router.push(`/admin/post-topics/${topic.id}/edit`)} className="w-full flex items-center gap-3 px-4 py-4 text-[15px] font-bold text-blue-600 hover:bg-blue-50 border-b border-slate-50 transition-colors">
                                     <Pencil className="w-5 h-5" /> Chỉnh sửa
                                   </button>
                                   <button onClick={() => { handleDelete(topic.id); setOpenMenuId(null) }} className="w-full flex items-center gap-3 px-4 py-4 text-[15px] font-bold text-rose-600 hover:bg-rose-50 transition-colors last:rounded-b-2xl">
                                     <Trash2 className="w-5 h-5" /> Xóa hoàn toàn
                                   </button>
                                </div>
                             )}
                          </div>
