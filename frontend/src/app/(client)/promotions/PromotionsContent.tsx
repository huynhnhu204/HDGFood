...
import { 
  Filter, X, ChevronDown, ShoppingBag, 
  Search, SlidersHorizontal, Tag, Flame
} from 'lucide-react'
...
function PromotionsSkeleton() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
      {[1, 2, 3, 4, 5, 6].map(i => (
        <div key={i} className="bg-white rounded-[2.5rem] border border-slate-50 p-6 space-y-6 animate-pulse">
           ...
           <div className="h-12 bg-slate-50 rounded-full w-full" />
        </div>
      ))}
    </div>
  )
    ...
    <div className="min-h-screen bg-white">
      <div className="container mx-auto px-4 py-12 lg:py-16">
         <div className="flex flex-col lg:flex-row gap-12 lg:gap-16">
            ...
            <aside className="hidden lg:block lg:w-1/4 space-y-16">
                  ...
                  <div className="flex flex-col gap-2.5">
                     {DISCOUNT_FILTERS.map((item) => (
                        <button 
                           ...
                           className={`group flex items-center justify-between px-7 py-5 rounded-3xl text-[11px] font-black uppercase tracking-widest transition-all duration-300
                             ${filters.discount_range === item.value 
                               ? 'bg-[#ed2a2a] text-white shadow-2xl shadow-red-500/30' 
               ...
               <div className="space-y-10">
                  ...
                  <div className="space-y-2">
                     <button 
                        ...
                        className={`w-full text-left px-7 py-4 rounded-2xl text-xs font-black uppercase tracking-widest transition-all ${!filters.category ? 'text-[#ed2a2a] bg-red-50 shadow-sm' : 'text-slate-400 hover:bg-slate-50 hover:text-slate-600'}`}
                     >
                        Tất cả món ngon
                     </button>
                     {categories.map((cat) => (
                        <button 
                           ...
                           className={`w-full text-left px-7 py-4 rounded-2xl text-xs font-black uppercase tracking-widest transition-all ${filters.category === cat.id.toString() ? 'text-[#ed2a2a] bg-red-50 shadow-sm' : 'text-slate-400 hover:bg-slate-50 hover:text-slate-600'}`}
                        >
                           {cat.name}
                        </button>
                     ))}
               ...
               <div className="relative p-10 bg-slate-900 rounded-[3.5rem] overflow-hidden group shadow-2xl">
                  ...
                  <div className="relative z-10 space-y-6">
                     <h5 className="text-white text-2xl font-black italic uppercase leading-none tracking-tighter">Freeship tận cửa từ 200k</h5>
                     ...
                     <Link href="/products" className="flex items-center justify-center h-14 bg-[#ed2a2a] text-white text-[10px] font-black uppercase tracking-widest rounded-2xl shadow-xl active:scale-95 transition-all">Đặt món ngay</Link>
                  </div>
               </div>
            </aside>
            ...
            <main className="flex-1 space-y-12">
               ...
               {/* Controls Bar */}
               <div className="flex flex-col sm:flex-row items-center justify-between gap-6 pb-10 border-b border-slate-100">
                  <div className="flex items-center gap-4 w-full sm:w-auto">
                     <button 
                        onClick={() => setIsFilterOpen(true)}
                        className="lg:hidden flex-1 sm:flex-none flex items-center justify-center gap-3 px-8 py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl"
                     >
                        <Filter size={16} />
                        Bộ lọc ngay
                     </button>
                     <div className="hidden sm:flex items-center gap-4 bg-slate-50/50 border border-slate-100/50 px-6 py-4 rounded-3xl shadow-sm">
                  ...
                  <div className="flex items-center gap-4 w-full sm:w-auto">
                     ...
                     <select 
                        ...
                        className="flex-1 sm:flex-none bg-transparent py-2 text-[11px] font-black uppercase tracking-widest focus:text-[#ed2a2a] transition-colors outline-none cursor-pointer border-b border-slate-100"
                     >
               ...
               {loading ? (
                  ...
                  <div className="py-40 text-center space-y-10 bg-slate-50/50 rounded-[4rem] border border-dashed border-slate-200">
                     <div className="relative inline-flex flex-col items-center">
                        <div className="w-28 h-28 bg-white rounded-[2.5rem] flex items-center justify-center text-[#ed2a2a] shadow-2xl relative z-10 rotate-12">
                           <Search size={36} />
                        </div>
                        <motion.div 
                     ...
                     <div className="space-y-4 px-6">
                        <h3 className="text-3xl font-black text-slate-900 tracking-tighter uppercase italic leading-none">Hết deal hời rồi Như ơi!</h3>
                     ...
                     <button 
                        ...
                        className="px-12 py-5 bg-slate-900 text-white rounded-[2rem] text-[10px] font-black uppercase tracking-[0.25em] shadow-2xl hover:bg-[#ed2a2a] transition-all active:scale-95 hover:shadow-red-500/20"
                     >
                        Xem tất cả món ngon
                     </button>
                  </div>
               ...
               {hasMore && (
                  <div className="flex justify-center pt-20 pb-10">
                     <button 
                        ...
                        className="group relative px-14 py-5 bg-white border border-slate-200 text-slate-900 rounded-[3rem] text-[10px] font-black uppercase tracking-[0.4em] overflow-hidden shadow-sm hover:shadow-2xl transition-all disabled:opacity-50 active:scale-95"
                     >
      ...
      <Transition show={isFilterOpen} as={Suspense}>
        <Dialog as="div" className="relative z-[100] lg:hidden" onClose={setIsFilterOpen}>
          ...
          <div className="fixed inset-0 flex justify-end">
            ...
            >
              <Dialog.Panel className="relative ml-auto flex h-full w-full max-w-[340px] flex-col overflow-y-auto bg-white p-12 shadow-2xl">
                ...
                <div className="space-y-16">
                      ...
                      <div className="flex flex-col gap-3">
                         {DISCOUNT_FILTERS.map(item => (
                            <button 
                               ...
                               className={`px-8 py-6 rounded-[2.5rem] text-[10px] font-black uppercase tracking-[0.2em] text-left transition-all ${filters.discount_range === item.value ? 'bg-[#ed2a2a] text-white shadow-2xl shadow-red-500/20 scale-105' : 'bg-slate-50 text-slate-500 hover:bg-slate-100'}`}
                            >
                               {item.label}
                            </button>
                         ))}
                   ...
                   <div className="space-y-8">
                      ...
                      <div className="grid grid-cols-1 gap-3">
                         <button 
                           ...
                           className={`px-8 py-5 rounded-[2.5rem] text-[10px] font-black uppercase tracking-widest transition-all ${!filters.category ? 'bg-slate-900 text-white shadow-xl' : 'border border-slate-100 text-slate-400 hover:bg-slate-50'}`}
                         >
                            Tất cả món ngon
                         </button>
                         {categories.map(cat => (
                            <button 
                               ...
                               className={`px-8 py-5 rounded-[2.5rem] text-[10px] font-black uppercase tracking-widest transition-all ${filters.category === cat.id.toString() ? 'bg-slate-900 text-white shadow-xl' : 'border border-slate-100 text-slate-400 hover:bg-slate-50'}`}
                            >
                               {cat.name}
                            </button>
                         ))}
                   ...
                   <button 
                      ...
                      className="w-full py-6 bg-white border-2 border-slate-900 text-slate-900 rounded-[2.5rem] text-[10px] font-black uppercase tracking-widest hover:bg-slate-900 hover:text-white transition-all shadow-xl shadow-black/5"
                   >
                      Cài đặt lại
                   </button>
                </div>
